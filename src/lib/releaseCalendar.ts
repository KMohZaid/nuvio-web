...
/** Plain dates stay date-only; timestamped releases use the viewer's day. */
...
export function buildReleaseCalendar(metas: Meta[]): ReleaseCalendarItem[] {
  const unique = new Map<string, ReleaseCalendarItem>();
  for (const meta of metas) {
    if (meta.type.toLowerCase() === "movie") {
...
export function monthCells(
  ...
  const leading = weekStartsOnMonday ? (weekday + 6) % 7 : weekday;
