...
import {
  ...
  localReleaseDate,
  monthCells,
} from "../src/lib/releaseCalendar.ts";
...
test("plain release dates are validated and month cells align to weekdays", () => {
  ...
  const mondayCells = monthCells(2026, 7, true);
  assert.equal(mondayCells.slice(0, 5).every((day) => day === null), true);
  assert.equal(mondayCells[5], 1);
});
